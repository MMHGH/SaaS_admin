          ...
          <el-table :data="tableData" size="medium" :header-cell-style="{backgroundColor: '#f2f2f2'}">
            ...
            <el-table-column align="center" label="设置">
              <template slot-scope="scope">
                ...
                <el-button :form="scope.row.id" native-type="submit" size="medium" type="text"
                           @click.stop.prevent="updateOrganizationPowerConfig(scope.row.privilegeId, scope.row.userLvPrivilegeId, scope.row.form.updatedQuantity, $refs['form' + scope.row.privilegeId])">保存</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-pagination
  ...
  export default {
    ...
    methods: {
      ...
      getOrganizationPowerConfigList(callback){
        this.$service.post({
          url: this.$api.platformUserLevelDefinition.getOrganizationPowerConfigList,
          params: {
      ...
      // 修改单个平台用户类别的组织权益配置
      updateOrganizationPowerConfig(id, userLvPrivilegeId, updatedQuantity, ref){
